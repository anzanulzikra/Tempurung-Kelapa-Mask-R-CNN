...
from tensorflow.keras.applications import MobileNet as MobileNetV1
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    negative_indices = tf.where(tf.math.logical_and(
    ...
    def build(self, mode, config: Config):
        ...
        if callable(config.BACKBONE):
            ...
            if config.BACKBONE in ["resnet50", "resnet101"]:
                ...
                backbone = MobileNetV1(input_shape=config.BACKBONE_INPUT_SHAPE,
                                       alpha=config.MOBILENET_ALPHA,
                                       depth_multiplier=config.MOBILENET_DEPTH_MULTIPLIER,
                                       include_top=False,
                                       pooling=None)
                C5 = backbone.layers[-1].output
                C4 = backbone.get_layer('conv_pw_11_relu').output
                C3 = backbone.get_layer('conv_pw_5_relu').output
                C2 = backbone.get_layer('conv_pw_2_relu').output
            elif config.BACKBONE == "mobilenetv2":
                backbone = MobileNetV2(input_shape=config.BACKBONE_INPUT_SHAPE,
                                       alpha=config.MOBILENET_ALPHA,
                                       depth_multiplier=config.MOBILENET_DEPTH_MULTIPLIER,
                                       include_top=False,
                                       pooling=None)
                C5 = backbone.layers[-1].output
                C4 = backbone.get_layer('block_12_expand_relu').output
                C3 = backbone.get_layer('block_6_expand_relu').output
                C2 = backbone.get_layer('block_2_expand_relu').output
            else:
                raise ValueError("Unsupported backbone: ", config.BACKBONE)
        # Top-down Layers
